...
class JavaRuntimePlugin(snapcraft.BasePlugin):
    ...
    def build(self):
        ...
        # drop in java setup script
        f = open(self.installdir + '/set-java-env.sh', 'w')
        f.write("#!/bin/sh\n")
        if self.zulu:
            f.write('JAVA_HOME=$SNAP/jre\n')
            f.write('PATH=$SNAP/jre/bin:$PATH')
        else:
            f.write('JAVA_HOME=$SNAP/usr/lib/jvm/java-8-openjdk-%s' % self.project.deb_arch)
            f.write('PATH=$SNAP/usr/lib/jvm/java-8-openjdk-%s/bin:$PATH' % self.project.deb_arch)

        # for item in self.env(self, '$SNAP'):
        #     f.write("%s\n" % item)
